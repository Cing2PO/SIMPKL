...
@section('title', 'Edit Placement - SIMPKL')
@section('page_title', 'Edit Placement')
@section('page_subtitle', 'Update placement details')
...
@section('content')
    <div class="max-w-2xl mx-auto">
        ...
        <div class="bg-white rounded-lg shadow overflow-hidden">
            <form method="POST" action="{{ route('placements.update', $placement) }}" class="divide-y divide-gray-200">
                @csrf
                <div class="px-6 py-8 space-y-6">
                        ...
                        <div>
                            ...
                            <select id="institution_id" name="institution_id" class="w-full px-3 py-2 border @error('institution_id') border-red-500 @else border-gray-300 @enderror rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500" required>
                                ...
                                @foreach($institutions as $institution)
                                    <option value="{{ $institution->id }}" @selected(old('institution_id', $placement->institution_id) == $institution->id)>{{ $institution->name }}</option>
                                @endforeach
                            </select>
                    ...
                    <div>
                        ...
                        <select id="student_id" name="student_id" class="w-full px-3 py-2 border @error('student_id') border-red-500 @else border-gray-300 @enderror rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500" required>
                            ...
                            @foreach($students as $student)
                                <option value="{{ $student->id }}" @selected(old('student_id', $placement->student_id) == $student->id)>{{ $student->name }} ({{ $student->email }})</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select id="mentor_id" name="mentor_id" class="w-full px-3 py-2 border @error('mentor_id') border-red-500 @else border-gray-300 @enderror rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500" required>
                            ...
                            @foreach($mentors as $mentor)
                                <option value="{{ $mentor->id }}" @selected(old('mentor_id', $placement->mentor_id) == $mentor->id)>{{ $mentor->name }} ({{ $mentor->email }})</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select id="status" name="status" class="w-full px-3 py-2 border @error('status') border-red-500 @else border-gray-300 @enderror rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500" required>
                            <option value="pending" @selected(old('status', $placement->status) === 'pending')>Pending</option>
                            <option value="accepted" @selected(old('status', $placement->status) === 'accepted')>Accepted</option>
                            <option value="rejected" @selected(old('status', $placement->status) === 'rejected')>Rejected</option>
                        </select>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="date" id="start_date" name="start_date" value="{{ old('start_date', $placement->start_date ? \Carbon\Carbon::parse($placement->start_date)->format('Y-m-d') : '') }}" class="w-full px-3 py-2 border @error('start_date') border-red-500 @else border-gray-300 @enderror rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500" required>
                        ...
                        <div>
                            ...
                            <input type="date" id="end_date" name="end_date" value="{{ old('end_date', $placement->end_date ? \Carbon\Carbon::parse($placement->end_date)->format('Y-m-d') : '') }}" class="w-full px-3 py-2 border @error('end_date') border-red-500 @else border-gray-300 @enderror rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500" required>
                ...
                <div class="flex gap-4 justify-end px-6 py-4 bg-gray-50">
                    ...
                    <button type="submit" class="inline-flex items-center px-4 py-2 text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700">Update Placement</button>
                </div>
            </form>
        </div>
    </div>
