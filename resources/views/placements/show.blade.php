...
@section('page_subtitle', 'View placement information and attendance')
        ...
        <div class="bg-white rounded-lg shadow overflow-hidden mb-8">
            ...
            <div class="px-6 py-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <div class="mt-1 text-gray-900">{{ $placement->start_date ? \Carbon\Carbon::parse($placement->start_date)->format('d M Y') : '-' }}</div>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-500">Tanggal Selesai</label>
                    <div class="mt-1 text-gray-900">{{ $placement->end_date ? \Carbon\Carbon::parse($placement->end_date)->format('d M Y') : '-' }}</div>
                </div>
            </div>
        </div>
                    ...
                    @if(!$todayAttendance)
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <form method="POST" action="{{ route('attendances.checkIn', $placement) }}">
                                @csrf
                                <button type="submit" class="inline-flex items-center px-5 py-2.5 text-sm font-medium rounded-md text-white bg-green-600 hover:bg-green-700 shadow-sm">
                                    ✓ Check In
                                </button>
                            </form>
                        </div>
                    @elseif(!$todayAttendance->clock_out)
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <form method="POST" action="{{ route('attendances.checkOut', $todayAttendance) }}">
                                @csrf
                                <button type="submit" class="inline-flex items-center px-5 py-2.5 text-sm font-medium rounded-md text-white bg-red-600 hover:bg-red-700 shadow-sm">
                                    ✕ Check Out
                                </button>
                            </form>
                        </div>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse($attendances as $attendance)
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4 text-sm text-gray-900">{{ \Carbon\Carbon::parse($attendance->date)->format('d M Y') }}</td>
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-4 text-center text-sm text-gray-500">Belum ada riwayat absensi.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
