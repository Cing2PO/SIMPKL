...
@section('content')
    <div class="max-w-2xl mx-auto">
        ...
        <div class="bg-white rounded-lg shadow overflow-hidden">
            <form method="POST" action="{{ route('logbooks.update', $logbook->id) }}" class="divide-y divide-gray-200">
                @csrf
                <div class="px-6 py-8 space-y-6">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">ID</label>
                        <input type="text" value="{{ $logbook->id }}" class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm bg-gray-50 text-gray-600 cursor-not-allowed" disabled>
                    </div>
                    <div>
                        ...
                        <select id="placement_id" name="placement_id" class="w-full px-3 py-2 border @error('placement_id') border-red-500 @else border-gray-300 @enderror rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500" required>
                            ...
                            @foreach($placements as $placement)
                                <option value="{{ $placement->id }}" @selected(old('placement_id', $logbook->placement_id) == $placement->id)>{{ $placement->user?->name ?? 'Unknown' }} - {{ $placement->institution?->name ?? 'Unknown' }}</option>
                            @endforeach
                        </select>
